#if !defined(INSTRUCTIONSCOPY_H)
#define INSTRUCTIONSCOPY_H

const int MAX_INSTRUCTIONS_COPY = 5000;
const int MAX_DATA_COPY = 5000;

class InstructionsClassCopy
{
public:
	...
	InstructionsClassCopy();		 // The constructor initializes the array of machine code to look like a function
...
private:
	static unsigned char mCode[MAX_INSTRUCTIONS_COPY]; // NOTE: Using an stl vector does not work, because they relocate their array sometimes.
	int mCurrent;
	int mData[MAX_DATA_COPY];
};
