...
unsigned char InstructionsClassCopy::mCode[MAX_INSTRUCTIONS_COPY] = {0};

// A location to store an integer that is about to be printed.
int InstructionsClassCopy::gPrintInteger = 0;
int InstructionsClassCopy::gReadInteger = 0;

void HelperPrintInteger(void);

void InstructionsClassCopy::Encode(unsigned char c)
{
	if (mCurrent < MAX_INSTRUCTIONS_COPY)
		mCode[mCurrent++] = c;
	else
	{
		cerr << "Error. Used up all " << MAX_INSTRUCTIONS_COPY << " instructions." << endl;
		exit(1);
	}
}

void InstructionsClassCopy::Encode(int x)
{
	*((int *)(&(mCode[mCurrent]))) = x;
	mCurrent += 4;
}

void InstructionsClassCopy::Encode(long long x)
{
	*((long long *)(&(mCode[mCurrent]))) = x;
	mCurrent += 8;
}

void InstructionsClassCopy::Encode(void *p)
{
...
InstructionsClassCopy::InstructionsClassCopy()
{
	void *p = NULL;
	int pointerSize = sizeof(p);
	if (pointerSize == 4)
...
void InstructionsClassCopy::Finish()
{
	Encode(POP_EBP);
	Encode(NEAR_RET);
...
void InstructionsClassCopy::Execute()
{
	cout << "About to Execute the machine code..." << endl;
	void *ptr = InstructionsClassCopy::mCode;
	void (*f)(void);
	f = (void (*)(void))ptr;
	f();
...
void InstructionsClassCopy::PrintAllMachineCodes()
{
	for (int i = 0; i < mCurrent; i++)
	{
		printf("HEX: %2x  Decimal: %3i\n", (int)InstructionsClassCopy::mCode[i], (int)InstructionsClassCopy::mCode[i]);
	}
}

void InstructionsClassCopy::PushValue(int value)
{
...
void InstructionsClassCopy::Call(void *function_address)
{
	unsigned char *a1 = (unsigned char *)function_address;
	unsigned char *a2 = (unsigned char *)(&InstructionsClassCopy::mCode[mCurrent + 5]);
...
void HelperPrintInteger(void)
{
	// printf("%i ", InstructionsClassCopy::gPrintInteger);
	cout << InstructionsClassCopy::gPrintInteger;
}

void InstructionsClassCopy::PopAndWrite()
{
	Encode(POP_EAX);
	Encode(EAX_TO_MEM);
...
// in the mData array.
int *InstructionsClassCopy::GetMem(int index)
{
	if (index >= MAX_DATA_COPY)
	{
		cerr << "Error in GetMem. index is " << index << ", but NUM_DATA is only " << MAX_DATA_COPY << endl;
		exit(1);
	}
	return &(mData[index]);
}

void InstructionsClassCopy::PushVariable(unsigned int index)
{
...
void InstructionsClassCopy::PopAndStore(unsigned int index)
{
	int *variable_address = GetMem(index);
	Encode(POP_EAX);
	Encode(EAX_TO_MEM);
...
void InstructionsClassCopy::WriteEndl()
{
	Call((void *)HelperPrintEndl);
}

void InstructionsClassCopy::PopPopAddPush()
{
	Encode(POP_EBX);
	Encode(POP_EAX);
	Encode(ADD_EAX_EBX1);
...
void InstructionsClassCopy::PopPopSubPush()
{
	Encode(POP_EBX);
	Encode(POP_EAX);
	Encode(SUB_EAX_EBX1);
...
void InstructionsClassCopy::PopPopMulPush()
{
	Encode(POP_EBX);
	Encode(POP_EAX);
	Encode(MUL_EAX_EBX1);
...
void InstructionsClassCopy::PopPopDivPush()
{
	Encode(POP_EBX);
	Encode(POP_EAX);
...
void InstructionsClassCopy::PopPopComparePush(unsigned char relational_operator)
{
	Encode(POP_EBX);
	Encode(POP_EAX);
	Encode(CMP_EAX_EBX1);
...
void InstructionsClassCopy::PopPopLessPush()
{
	PopPopComparePush(JL);
}

void InstructionsClassCopy::PopPopLessEqualPush()
{
	PopPopComparePush(JLE);
}

void InstructionsClassCopy::PopPopGreaterPush()
{
	PopPopComparePush(JG);
}

void InstructionsClassCopy::PopPopGreaterEqualPush()
{
	PopPopComparePush(JGE);
}

void InstructionsClassCopy::PopPopEqualPush()
{
	PopPopComparePush(JE);
}

void InstructionsClassCopy::PopPopNotEqualPush()
{
	PopPopComparePush(JNE);
}

void InstructionsClassCopy::PopPopAndPush()
{
	Encode(IMMEDIATE_TO_EAX); // load A register with 0
	Encode(0);
...
{
	cout << ">> ";
	cin >> InstructionsClassCopy::gReadInteger;
}

void InstructionsClassCopy::ReadAndStoreVariable(unsigned int index)
{
	int *variable_address = GetMem(index);
...
void InstructionsClassCopy::PopPopOrPush()
{
	Encode(IMMEDIATE_TO_EAX); // load A register with 0
	Encode(0);
	Encode(POP_EBX); // load B register with stack item 2
...
unsigned char *InstructionsClassCopy::SkipIfZeroStack()
{
	Encode(POP_EBX);
	Encode(IMMEDIATE_TO_EAX); // load A register with 0
	Encode(0);
...
void InstructionsClassCopy::SetOffset(unsigned char *codeAddress, int offset)
{
	*((int *)codeAddress) = offset;
}

unsigned char *InstructionsClassCopy::GetAddress()
{
	return &(InstructionsClassCopy::mCode[mCurrent]);
}

unsigned char *InstructionsClassCopy::Jump()
{
